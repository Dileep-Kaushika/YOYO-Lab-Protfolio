...
// Enhance selected native <select> elements with a custom dark dropdown
(function () {
  const idsToEnhance = ['reviews-sort', 'reviews-filter', 'contact-service'];

  // Keep native selects on touch-only devices (iOS/Android pickers are good)
  const isCoarse = window.matchMedia && window.matchMedia('(pointer: coarse)').matches;

  function enhanceSelect(sel) {
    if (!sel || sel.dataset.enhanced || isCoarse) return;
    ...
    const label = document.createTextNode(sel.options[sel.selectedIndex]?.text || sel.options[0]?.text || '');
    ...
    Array.from(sel.options).forEach((opt, i) => {
      ...
      li.addEventListener('click', () => { selectIndex(i); close(); btn.focus(); });
      list.appendChild(li);
    });
    ...
    function open() {
      ...
      const current = list.children[sel.selectedIndex];
    ...
    function onKeydown(e) {
      ...
      else if (e.key === 'Enter') { e.preventDefault(); close(); btn.focus(); }
    }
    function step(delta) {
    ...
    function selectIndex(i) {
      sel.selectedIndex = i;
      label.nodeValue = sel.options[i].text;
      Array.from(list.children).forEach((li, ix) => li.setAttribute('aria-selected', String(ix === i)));
      // fire change for any listeners (e.g., filtering)
  ...
  document.addEventListener('DOMContentLoaded', () => {
    idsToEnhance.forEach(id => enhanceSelect(document.getElementById(id)));
  });
})();
